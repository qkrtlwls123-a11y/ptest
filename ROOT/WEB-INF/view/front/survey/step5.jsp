		...
		<div class="orientation-wrap infor-wrap">
			<div class="sub-title">
				...
				<div class="progress">
					...
					<div class="part part3 on"></div>
					<div class="part part4"></div>
				</div>
			</div>
			<div class="sub-text">
			<c:if test="${currentPage eq 8}">
				<p class="pink">본 설문은 자신의 성향에 대한 것이므로 정답이 없습니다.</p>
				<p class="pink">한 문장에 대해 너무 깊이 생각하기 보다는 자연스럽고 솔직하게 응답하시기 바랍니다.</p>
				<p>각 문장을 읽고 자신의 모습과 가까운 곳에 체크해 주세요.</p>
			</c:if>
			</div>
			<div class="research-content">
			
				<!-- <h4>* 각 문장을 읽고 자신의 모습과 가까운 곳에 체크해주세요.</h4> -->
						...
						<c:forEach var="item2" items="${item.itemList}" varStatus="status2">
							<c:choose>
								<c:when test="${item2.confirm_yn eq 'Y'}">
									<tr id="item${item2.item_id}" class="bg-c">
										...
										<td>
											<div class="radio-wrap2">
												...
												<label for="item3${item2.item_id}">
													<span class="chk-icon"></span>
													<span class="m-block">약간 아니다</span>
												</label>
											</div>
										</td>
										<td>
										...
										<td>
											<div class="radio-wrap2">
												...
												<label for="item5${item2.item_id}">
													<span class="chk-icon"></span>
													<span class="m-block">그렇다</span>
												</label>
											</div>
										</td>
										<td>
											<div class="radio-wrap2">
												<input id="item6${item2.item_id}" disabled="disabled" value="6" type="radio" name="item${item2.item_id}" <c:choose><c:when test="${empty item2.answer_no}"></c:when><c:when test="${item2.item_no eq 6}">checked="checked"</c:when></c:choose>>
												<label for="item6${item2.item_id}">
													<span class="chk-icon"></span>
							...
							<c:otherwise>
									<tr id="item${item2.item_id}" class="bg-c">
										...
										<td>
											<div class="radio-wrap2">
												<input id="item6${item2.item_id}" onclick="Survey.changeItemNo('${item2.item_id}','6')" value="6" type="radio" name="item${item2.item_id}" <c:choose><c:when test="${empty item2.answer_no}"></c:when><c:when test="${item2.item_no eq 6}">checked="checked"</c:when></c:choose>>
												<label for="item6${item2.item_id}">
													<span class="chk-icon"></span>
													<span class="m-block">매우 그렇다</span>
												</label>
...
<form action="" id="Step5Form" style="display: none;">

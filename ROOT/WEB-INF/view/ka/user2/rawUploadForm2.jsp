...
function rowRegAction(){
	...
	$("#prog").show();
	$("#rowFileForm").attr("action","/ka/user2/goRequestAction.do");
	$("#rowFileForm").ajaxSubmit({
		async:false,
		success:function(data){
			if(data.rcode > 0){
				alert("업로드 완료.\n총 "+data.total+"건\nupdate "+data.delcnt+"건");
				setTimeout(function() {
					$("#prog").hide();
					COMM_POPUP.close();
	...
	<div id="content" class="popup">
		...
		<form method="post" action="/ka/user2/goRequestAction.do" id="rowFileForm" enctype="multipart/form-data">

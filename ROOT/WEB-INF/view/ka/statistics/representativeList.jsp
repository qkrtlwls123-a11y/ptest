	...
	var Survey = {
			...
			setStatisticsList : function(localdata){ //로딩된 데이터를 받아서 그리드 생성
				...
				$.each(localdata.list, function(idx, item){
		            ...
		            if(item.representative_yn == "Y"){
		            	tag += 	"<td><input name=\"radio_representative\" type=\"radio\" checked=\"checked\" onclick=\"Survey.surveyAction('"+item.survey_id+"','"+item.responder_id+"','representative',this)\"/></td>";
		            }else{
		            	tag += 	"<td><input name=\"radio_representative\" type=\"radio\" onclick=\"Survey.surveyAction('"+item.survey_id+"','"+item.responder_id+"','representative',this)\"/></td>";
		            }
			...
			statisticsAction : function(){ //대표구성원유사도처리
				...
				if(s_survey_id == undefined || s_survey_id == null || s_survey_id == "" ){
					alert("먼저 진단 검사를 선택하여 주시기 바랍니다.");
					return;
				}
				let params = {};
				params.survey_id = s_survey_id;
				
				var url = "/ka/statistics/representativeAction.do";
				$.ajax({
					url			: url,
					type		: "post",
					data		: $.param(params),
					success		: function(data){
						if(data.rcode == 200){
							...
							Survey.getStatisticsList(Survey.surveyParams["pageNo"]);
						}else if(data.rcode == 401){
							alert("먼저 진단 검사를 선택하여 주시기 바랍니다.");
						}else{
							alert("처리중 문제가 있었습니다.");
						}
	...
	<div id="container">	
		...
		<div id="content" class="admin_con">
			...
			<div class="btn_space_listT">
				<input type="button" value="통계실행" onclick="Survey.statisticsAction('regist')"/>

		...
		<input type="hidden" id="id" name="id" value="${view.id}"/>
			<table>
				...
				<tr>
					...
					<td>
						<select name="user_type">
							<option value="N" <c:if test="${view.user_type eq 'N'}">selected="selected"</c:if>>검사자</option>
							<option value="A" <c:if test="${view.user_type eq 'A'}">selected="selected"</c:if>>관리자</option>
							<option value="S" <c:if test="${view.user_type eq 'S'}">selected="selected"</c:if>>고객사</option>
						</select>
					</td>
				</tr>
				<tr>
					<th>회사지역</th>
					<td colspan="3">
						<input type="text" name="area" value="${view.area}" class="w600" maxlength="100"/>
					</td>
				</tr>
				<tr>
					<th>성별</th>
					...
					<th>출생연도</th>
					<td>
						<input type="text" name="year_birth" value="${view.year_birth}" class="w100" maxlength="4"/>
					</td>
				</tr>
				<tr>
					<th>재직구분</th>
				...
				<tr>
					...
					<th>설문상태</th>
					<td>
						<select name="survey_state">						
							<option value="">직무를 선택하세요.</option>
						<c:forEach var="item" items="${surveyStateGroup}">
							<option value="${item.code}" <c:if test="${view.survey_state eq item.code}">selected</c:if>>${item.code_nm}</option>
						</c:forEach>
						</select>
					</td>
				</tr>

...
/* 		function createEditor(){
		...
		<div id="content" class="popup">
				...
				<form id="SurveyForm" method="post" action="/ka/survey/goRecruitSurveyAction.do">
				<input type="hidden" name="action_mode" value="${box.action_mode}"/>
					<table>
						<tbody>
						<tr>
							<th style="width:15%;">진단검사종류</th>
							<td style="width:85%;" colspan="3">
								<select name="survey_id">
									<option value="">진단검사를 선택하세요.</option>
									<c:forEach var="item" items="${surveyGroup}" varStatus="status">
										<option value="${item.id}" <c:if test="${view.survey_id eq item.id}">selected</c:if>>${item.survey_nm}</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							<th style="width:15%;">채용공고명</th>
							<td style="width:35%;">
									<input type="hidden" name="id" value="${view.id}"/>
									<input type="text" class="yellow w300" name="recruit_nm" value="${view.recruit_nm}"/>
							</td>

...
$(document).ready(function() {
	...
	function fn_insertFile(targetName){
		...
		$.ajax({
			type : "POST",
			url : "/voomerang/file/uploadProductImage.json",
			data : formData,
			processData: false,
			contentType: false,
			success : function(result) {
				if(result.resultCode == "success"){
					...
					for(var i=0; i<1; i++){
						...
						htmlString += '<img src="${pageContext.request.contextPath}/voomerang/file/down/image/'+fileId+'" alt="" style="width:100%;">';
						if(targetName == "3"){
							$("#product_thumbnail3").val(fileId);
						}else if(targetName == "2"){
							$("#product_thumbnail2").val(fileId);
						}else{
							$("#product_thumbnail").val(fileId);
						}
    ...
    $('#add-item').on( 'click', function (e) {
		...
		$(".insert-item").each(function(check){
			...
			if($(this).hasClass("fileupload")){
				...
				}else if($(this).attr("name") == 'company_id'){
					orderNo += "<br>"+$(this).val();
					item = orderNo;
					itemArray.push(item);
				}else{
		...
		$.ajax({
			url : "/voomerang/insertProduct.json",
			type : "POST",
			data : params,
			success : function(result) {
				if (result.resultCode == "success") {
					...
					location.href = "/voomerang/siteList.do";
				} else {

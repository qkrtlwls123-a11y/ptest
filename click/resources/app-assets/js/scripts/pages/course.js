...
$(document).ready(function () {
    ...
    $(".list-group-item").on("click", function(){
    	...
    	$.ajax({
			url : "/click/selectCourseGroupList.json",
			type : "POST",
			data : params,
			success : function(result) {
				if (result.resultCode == "success") {
					var htmlString = '';
					for(var i=0;i<result.groupGroupList.length;i++){
						console.log(result.groupGroupList[i].group_id);
						htmlString += '<div class="card text-center m-05 group-card3" id="group_object_id_'+result.groupGroupList[i].group_id+'">'+
											'<input type="hidden" name="group_object" value="'+result.groupGroupList[i].group_id+'">'+
								         	'<div class="card-content">'+
									     		'<div class="card-body2">'+
									                '<span class="card-text bt-card">'+result.groupGroupList[i].group_name+'<button class="btn btn-primary btn-darken-3 ml-05 pd-05" onclick="javascript:fn_delete('+result.groupGroupList[i].group_id+');return false;"><i class="feather icon-x align-middle"></i></button></span>'+
									            '</div>'+
									        '</div>'+
									     '</div>';
					}
    					
					$("#groupGroupList").html(htmlString);
    ...
    $(".group-card").on("doubleTap", function(){
    	var groupId = $(this).find("input[type=hidden]").val();
		var groupName = $(this).find(".bt-card").text();
		var htmlString = '<div class="card text-center m-05 group-card3" id="group_object_id_'+groupId+'">'+
							'<input type="hidden" name="group_object" value="'+groupId+'">'+
				         	'<div class="card-content">'+
					     		'<div class="card-body2">'+
					                '<span class="card-text bt-card">'+groupName+'<button class="btn btn-primary btn-darken-3 ml-05 pd-05" onclick="javascript:fn_delete('+groupId+');return false;"><i class="feather icon-x align-middle"></i></button></span>'+
					            '</div>'+
					        '</div>'+
					     '</div>';
		var checkGroup = 0;
		...
		if(checkDuplicate){
			alert("이미 등록되어있는 그룹입니다.");
		}else{
			if(checkGroup == 0){
    			$("#courseGroupList").html(htmlString);
    		}else{
    			$("#courseGroupList").append(htmlString);
    		}
    		...
    		$("#course_group").val(course_group);
    		
    		var params = $("form[name=parameterVO]").serialize();
        	
        	$.ajax({
    			url : "/click/updateCourseGroup.json",
    			type : "POST",
    			data : params,
    			success : function(result) {
    ...
    $(".group-card").on("dblclick", function(){
		var groupId = $(this).find("input[type=hidden]").val();
		var groupName = $(this).find(".bt-card").text();
		var htmlString = '<div class="card text-center m-05 group-card3" id="group_object_id_'+groupId+'">'+
							'<input type="hidden" name="group_object" value="'+groupId+'">'+
...
function fn_update(){
	...
	$.ajax({
		...
		success : function(result) {
			if (result.resultCode == "success") {
				location.href="/click/groupList.do";
			} else {
				alert("시스템 오류 입니다.");
			}
		}

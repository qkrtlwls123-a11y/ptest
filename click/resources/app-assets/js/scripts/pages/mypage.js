...
function fn_updateUser(){
	...
	$.ajax({
		url : "/voomerang/updateUser.json",
		type : "POST",
		data : params,
		success : function(result) {
			if (result.resultCode == "success") {
				alert("수정되었습니다.");
				location.href = "/voomerang/mypage.do";
			} else {
				location.href = "/voomerang/mypage.do";
			}
		}
...
function fn_changePassword(){
	if($("#check_password").val() == $("#user_password").val()){
		if($("#user_password1").val() == $("#user_password2").val()){
			$("#change_password").val($("#user_password1").val());
			var params = $("form[name=parameterVO]").serialize();
			$.ajax({
				url : "/voomerang/changePassword.json",
				type : "POST",
				data : params,
				success : function(result) {
					if (result.resultCode == "success") {

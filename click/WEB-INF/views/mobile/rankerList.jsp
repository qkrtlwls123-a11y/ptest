...
<div class="wrap" style="background:#F6F6F6;">
	...
	<div class="areas" style="background:white;">
		<header class="header_top h260" style="background:white;">
			<p class="title" style="color:#000;">명예의 전당</p>
			<a href="${pageContext.request.contextPath}/mobile/rankingList.do" class="btn_left btn_back" style="z-index:1000;font-size:26px;line-height:80px;padding-left:15px;"><i class="feather icon-arrow-left"></i><em>이전 페이지로 돌아가기</em></a>
		</header>
		<div class="container">
			...
			<div class="rankingList ranker-area">
				<c:if test="${fn:length(rankingList) > 0}">
					<c:forEach var="rankingList" items="${rankingList}" varStatus="status">
						<div class="rankingListItem">
							<div class="rankingLeftArea">
								<div class="rankingNo">
									<img src="${pageContext.request.contextPath}/resources/mobile/images/rank.png" alt="랭커 이미지">
								</div>
				...
				<c:if test="${fn:length(rankingList) == 0}">
					<div class="no-ranker">명예의 전당에 오르신분이 없네요.</div>
				</c:if>
			</div>
		</div><!-- container -->
	</div><!-- areas -->

...
<input type="hidden" id="search_sensor_detail_type" name="search_sensor_detail_type" value="2"/>
<input type="hidden" id="search_date_type" name="search_date_type" value="MONTH" />
<input type="hidden" id="day_hour" name="day_hour" value=""/>
			...
			if(result.resultCode == "success"){
				if(0 < result.totalCnt){
	        		...
	        		for(var i=0; i<result.resultList.length;i++){
		        		...
		        		//data.push(parseFloat(result.resultList[i].rain_since_one_time));
		        		
		        		if( result.resultList[i].last1hour != "null" ){
			        		data.push(parseFloat(result.resultList[i].last1hour));
		        		}else{
			        		data.push(null);
		        		}
		        	}
	        		maxLast1hour = parseInt(result.resultList[0].maxLast1hour) + 20;
...
function fn_drawChart(){
	...
	Highcharts.chart('container', {
	    ...
	    series: [{
	        name: '<spring:message code="click.rainfall"/>',
	        data: data ,
	        zIndex : 5
            , pointStart: Date.UTC(dateY, dateM, dateD, timeH, timeI)
            , pointInterval: 3600 * 1000

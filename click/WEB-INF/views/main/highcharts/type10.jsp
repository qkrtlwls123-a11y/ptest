					...
					if(result.resultCode == "success"){
						...
						if(result.equipList.length > 0){
							for(var i=0;i<result.equipList.length;i++){
								if(result.equipList[i].equip_count > 0){
									if('en' == '${locale}'){
										var checkData = { name : result.equipList[i].equip_name_eng, y : parseInt(result.equipList[i].equip_count)};
									}else{
										var checkData = { name : result.equipList[i].equip_name, y : parseInt(result.equipList[i].equip_count)};
									}
									
									equipArray.push(checkData);
								}
							}
						}
						if(equipArray.length > 0){
							...
							if('en' == '${locale}'){
								$("#container").html('<p class="text-none">None</p>');
							}else{
								$("#container").html('<p class="text-none">없음</p>');
							}
						}
					}
				}
